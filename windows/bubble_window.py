...
class BubbleWindow(WindowBase):
    def __init__(self, root, x_pos, y_pos):
        ...
        self.font = ("Helvetica Neue", 10)  # ("San Francisco", 10)
    ...
    def set_balloons(self):
        ...
        hukidasi_height = 55
        if self.window_height < hukidasi_height:
            hukidasi_height = self.window_height / 2
        # 吹き出しの尾部となる三角形を描画
        triangle = [
            (300, hukidasi_height - 5),  # バルーンの右端から出るように調整
            (310, hukidasi_height),  # 三角形の先端をバルーンの中心線に合わせる
            (300, hukidasi_height + 5),
        ]
        ...
        self.canvas.create_oval(
            300 - radius * 2, 0, 300, radius * 2, fill=balloon_color, outline=balloon_color, tags="balloon"
        )  # 右上の角
        self.canvas.create_oval(
            0,
        ...
        self.canvas.create_rectangle(
            radius, 0, 300 - radius, self.window_height, fill=balloon_color, outline=balloon_color, tags="balloon"
        )  # 中央
        self.canvas.create_rectangle(
            0, radius, 300, self.window_height - radius, fill=balloon_color, outline=balloon_color, tags="balloon"
        )  # 上下
    ...
    def update_sns_posts(self):
        ...
        self.canvas.create_window(10, 10, window=label, anchor="nw", tags="post_text")
        ...
        # バルーンの高さがをlabel_height+20になるように調整し、再描画
